...
def calculate_learnable_loss(local_prototypes, local_queries, ground_truth_labels):
    ...
    # Ensure ground_truth_labels is one-hot encoded and reshape it to match probabilities
...
def federated_training_process(num_rounds, local_datasets, global_model, lambda1, lambda2, lambda3, apply_privacy=False):
    ...
    for t in range(num_rounds):
        ...
        for i in range(num_nodes):
            ...
            local_model = copy.deepcopy(global_model)  # Clone the global model for local adaptation
...
def load_data():
    # Adjusting the placeholder example to ensure the shape matches what PointNet++ expects:
    N = 100  # Number of samples
    P = 1024  # Number of points in each sample
    D = 3    # Number of dimensions for each point (e.g., x, y, z coordinates)
    C = 10   # Number of classes
    return [(torch.rand(N, P, D), torch.randint(0, 2, (N, C)).float()) for _ in range(5)]  # 5 nodes

local_datasets = load_data()
global_model = GlobalModel(input_dim=3, feature_dim=128, num_classes=10)
